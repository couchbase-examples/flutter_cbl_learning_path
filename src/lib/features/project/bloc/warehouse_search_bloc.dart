...
class WarehouseSearchBloc
    extends Bloc<WarehouseSearchEvent, WarehouseSearchState> {
  WarehouseSearchBloc(
  ...
  FutureOr<void> _onCityChanged(WarehouseSearchCityChangedEvent event,
      Emitter<WarehouseSearchState> emit) {
    emit(state.copyWith(
        searchCity: event.searchCity, status: FormEditorStatus.dataChanged));
  }

  FutureOr<void> _onStateChanged(WarehouseSearchStateChangedEvent event,
      Emitter<WarehouseSearchState> emit) {
    emit(state.copyWith(
  ...
  FutureOr<void> _onSelection(WarehouseSearchSelectionEvent event,
    ...
    if (warehouse != null) {
      ...
      emit(state.copyWith(
          error:
              'Warehouse should not be null when selected, but is for some strange reason.',
          status: FormEditorStatus.error));
    }
  }
}
