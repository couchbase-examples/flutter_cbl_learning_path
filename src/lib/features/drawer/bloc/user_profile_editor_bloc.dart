...
class UserProfileEditorBloc extends Bloc<UserProfileEditorEvent, UserProfileEditorState> {
    ...
    try {
      ...
      if (user.isNotEmpty && user.keys.length > 1) {
        ...
        if (user.containsKey('imageData')) {
          ...
          var rawData = await blob.content();
          ...
          emit(state.copyWith(status: FormEditorStatus.dataLoaded,
              ...
              lastName: surname,
              jobTitle: jobTitle,
              imageRaw: _getDefaultImageBase64()));
        }
      }
      else {
        emit(state.copyWith(status: FormEditorStatus.dataLoaded,
  ...
  Future<FutureOr<void>> _onSelectImageEvent(
      SelectImageEvent event,
      Emitter<UserProfileEditorState> emit) async {
    if(event.file != null){
      var imageBytes = await event.file?.readAsBytes();
      if (imageBytes != null && imageBytes.isNotEmpty){
        emit(state.copyWith(status: FormEditorStatus.dataChanged,
            imageRaw: imageBytes));
      }
    }
  }
